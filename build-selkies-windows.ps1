...
docker run --rm `
    ...
    ubuntu:$UbuntuVersion bash -c @"
        ...
        tar -czf /workspace/dist/selkies-gstreamer-web_v1.6.2+w.tar.gz src/ && \
...
if (Test-Path (Join-Path $DistDir "selkies-gstreamer-web_v1.6.2+w.tar.gz")) {
    Write-Host "✓ Web interface: selkies-gstreamer-web_v1.6.2+w.tar.gz" -ForegroundColor Green
} else {
...
if (-not $SkipGStreamer) {
    ...
    if (Test-Path $gstDockerfile) {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            $gstTarball = "gstreamer-selkies_gpl_v1.6.2+w_ubuntu${UbuntuVersion}_${arch}.tar.gz"
